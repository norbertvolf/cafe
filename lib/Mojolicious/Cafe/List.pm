...
sub list {    #Getter/setter for list of records
	my $self = shift;
	if ( scalar(@_) ) {
		$self->{_list} = shift;
...
sub push {     #add new value to list
	my $self = shift;
...
sub search {    #Return array with items corresponding, with parameters passed as hash
	...
	foreach my $r ( $self->list ) {
		if ( ref($r) eq 'HASH' ) {
			$equals = grep {
				(        ( defined( $r->{$_} ) && defined( $hash{$_} ) && $r->{$_} eq $hash{$_} )
					  || ( !defined( $r->{$_} ) && !defined( $hash{$_} ) ) )
			} keys(%hash);
		} else {
			$equals = grep {
				( ( defined( $r->$_ ) && defined( $hash{$_} ) && $r->$_ eq $hash{$_} ) || ( !defined( $r->$_ ) && !defined( $hash{$_} ) ) )
			} keys(%hash);
		}
...
sub count {    #Compute and return number of rows
	my ( $self, $force ) = @_;
...
sub query_compiled {    #Remove parameters and dynamically used SQL keywords from query
	my $self = shift;

	#Add limit and offset
	if ( defined( $self->limit ) && looks_like_number( $self->limit ) ) {
		my $limit_clause = $self->limit;
		$limit_clause = join( ' ', $self->limit, "OFFSET", $self->offset )
		  if ( defined( $self->offset ) && looks_like_number( $self->offset ) );
...
sub query_params {    #Prepare params for compiled query
	my $self = shift;
	my @params = map { $self->$_ } $self->query->parameters;
	$self->c->app->log->debug( "Query parameters: " . join( ', ', map { ( defined($_) ? qq("$_") : "NULL" ) } @params ) . "." )
	  if ( scalar(@params) );
	return (@params);
}

sub counter_params {    #Prepare params for compiled query
	my $self = shift;
	my @params = map { $self->$_ } $self->query->counter_parameters;
	$self->c->app->log->debug( "Counter parameters: " . join( ', ', map { ( defined($_) ? qq("$_") : "NULL" ) } @params ) . "." )
	  if ( scalar(@params) );
	return (@params);
}
